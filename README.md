...
- 출발지 기준 예상 소요시간을 요약 카드와 모바일 하단 시트에 크게 표시
...
- 실제 혼잡도 API 연결 전까지 장소 유형과 시간대 기반의 로컬 혼잡도 추정 적용
...
출발 기준 시간대를 바꾸면 시간대 보정이 다시 적용됩니다. 실제 경로 API가 없을 때의 예상 소요시간은 직선거리, 거리대별 우회율, 거리대별 평균 속도, 시간대 보정을 합친 로컬 추정값입니다. 실제 내비 경로/API가 붙기 전에는 신뢰도를 낮게 표시합니다.
...
TMAP 자동차 경로안내를 프록시에서 사용하려면 환경변수로 provider와 앱키를 설정합니다.

```powershell
$env:ROUTE_PROVIDER="tmap"
$env:TMAP_APP_KEY="여기에_TMAP_AppKey"
npm run route:dev
```
...
- TMAP 또는 다른 경로 API를 연결해 실제 예상 지연률 계산: `ROUTE_PROXY_URL` 어댑터와 개발용/TMAP 프록시 완료, 실제 TMAP AppKey 검증 필요
